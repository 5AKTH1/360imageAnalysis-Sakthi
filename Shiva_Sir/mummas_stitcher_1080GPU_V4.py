...
# GPU acceleration — falls back transparently to CPU if CUDA / CuPy not available
try:
    import cupy as cp
...
# ---------------------------------------------------------------------------
# V4: Config loader — reads config.yaml, falls back to code defaults silently
# ---------------------------------------------------------------------------
...
def _cfg(section, key, default):
    """Return config value from config.yaml or fall back to default."""
...
def process_and_undistort(frame, K_orig, D, xi, orig_size, fov_scale=0.4):
    try:
        ...
        # Precompute undistortion maps so we can use INTER_CUBIC (sharper than
        # the fixed bilinear used internally by omnidir.undistortImage)
...
class AssetDiscovery:
    @staticmethod
    def check_csv_contains_timestamp(csv_path, target_dt):
        try:
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                time_col = next((c for c in reader.fieldnames if c and ('timestamp_local' in c.lower() or 'local_timestamp' in c.lower())), None)
                if not time_col: return False
                
                first_time_str = None
                last_time_str = None
                
                for row in reader:
                    t_str = row.get(time_col, "").strip()
                    if not t_str: continue
                    clean_time_str = t_str.split('.')[0]
                    if not first_time_str: first_time_str = clean_time_str
                    last_time_str = clean_time_str
                ...
                if first_time_str and last_time_str:
                    ...
                    if (start_dt - timedelta(minutes=15)) <= target_dt <= (end_dt + timedelta(minutes=15)):
                        return True
                        
                    # Check 2: Date-Healed match (Fixes the DD/MM vs MM/DD issue internally)
                    try:
                        swapped_start = start_dt.replace(month=start_dt.day, day=start_dt.month)
                        swapped_end = end_dt.replace(month=end_dt.day, day=end_dt.month)
                        if (swapped_start - timedelta(minutes=15)) <= target_dt <= (swapped_end + timedelta(minutes=15)):
                            return True
                    except ValueError:
                        pass
    ...
    def search_for_timestamp(root_drive, target_dt, verbose=True):
        ...
        # 1. Search for IMU-GPS CSV (MASSIVE SPEEDUP APPLIED HERE)
            ...
            for root, dirs, files in os.walk(s_dir):
                for file in files:
                    if file.endswith(".csv"):
                        # FAST FILTER: Only open the CSV if its filename contains the exact or swapped date!
                        if target_csv_date in file or swapped_csv_date in file:
                            potential_csv = os.path.join(root, file)
                            if AssetDiscovery.check_csv_contains_timestamp(potential_csv, target_dt):
                                matched_csv = potential_csv
                                break
                if matched_csv: break
            if matched_csv: break
        ...
        if verbose: UI.info("Scanning for corresponding session_meta.json and calculating video duration...")
        matched_json_folder = None
        session_meta_data = None
            ...
            for root, dirs, files in os.walk(s_dir):
                if "session_meta.json" in files:
                    ...
                    try:
                            ...
                            if utc_str:
                                ...
                                if os.path.exists(vid_path):
                                    ...
                                    if frames > 0:
                                        ...
                                        # Standard Math Check
                                        if (start_ist - timedelta(minutes=5)) <= target_dt <= (end_ist + timedelta(minutes=5)):
                                            matched_json_folder = root
                                            session_meta_data = meta
                                            break
                                        
                                        # Date-Healed Math Check (Fixes the DD/MM swap for videos)
                                        try:
                                            swapped_start = start_ist.replace(month=start_ist.day, day=start_ist.month)
                                            swapped_end = end_ist.replace(month=end_ist.day, day=end_ist.month)
                                            if (swapped_start - timedelta(minutes=5)) <= target_dt <= (swapped_end + timedelta(minutes=5)):
                                                matched_json_folder = root
                                                session_meta_data = meta
                                                break
                        ...
                        continue
            if matched_json_folder: break

        if not matched_json_folder:
            if verbose: UI.error("meta.json files not available to map with the csv files for this timestamp.")
            return None, None, None

        missing_lenses = []
        for i in range(1, 7):
            lens_vid = os.path.join(matched_json_folder, f"LENS{i}", f"video_lens{i}.mp4")
        ...
        return matched_csv, matched_json_folder, session_meta_data
    ...
    try:
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                # Match to the correct session based on the rigorous IST conversions
                session = next((s for s in registry if (s["start_ist"] - timedelta(minutes=15)) <= ist_dt <= (s["end_ist"] + timedelta(minutes=15))), None)
                if not session: 
                    continue
                ...
                # Prevent negative frames if IMU timestamp triggers right before video
        ...
        # --- SMART MASTER MAPPING APPEND LOGIC ---
        file_exists = os.path.exists(master_mapping_path)
        existing_timestamps = set()
        
        # Read existing timestamps to prevent duplicate rows in the Master CSV
        if file_exists:
            try:
        ...
        # Append only the new rows to the Master Mapping
        if new_rows:
...
def find_sharpest_frame_offset(video_path, base_frame, offsets=list(range(0, 30, 5))):
...
def _blend_frames(warpeds, weights):
    ...
    Always returns a plain NumPy uint8 array ready for ffmpeg / cv2."""
    ...
    # Upload all warped frames to GPU (or keep as NumPy) once
    xwarpeds = [xp.asarray(w, dtype=xp.float32) for w in warpeds]
    xweights = [xp.asarray(w) for w in weights]

    # Brightness-match each lens to lens 0 in their overlap region
    ...
    # Download from GPU if needed and return NumPy uint8
    if _GPU:
        result = cp.asnumpy(result)
    return result.astype(np.uint8)


# --------------- GPU REMAP (CuPy bicubic, matches OpenCV INTER_CUBIC) ---------------

_BICUBIC_REMAP_KERNEL = None

def _get_bicubic_remap_kernel():
    """Lazily compile a CUDA kernel for Keys bicubic remap (a = -0.75)."""
        ...
        unsigned char* dst, int dst_h, int dst_w)
    {
        ...
        /* Out-of-bounds -> black (BORDER_CONSTANT equivalent) */
        ...
        /* Keys cubic weight (a = -0.75) — same as OpenCV INTER_CUBIC */
...
def _gpu_remap(frame_np, map_x_np, map_y_np, gpu_maps=None):
    """Remap a single frame using GPU bicubic interpolation.
    Returns CuPy array (stays on GPU) if available, else NumPy via cv2.remap."""
    if not _GPU:
    ...
    return dst  # stays on GPU for _blend_frames


def _upload_maps_to_gpu(maps_x, maps_y):
    """Upload remap coordinate arrays to GPU. Returns list of (gx, gy) tuples."""
    if not _GPU:
        return None
    return [(cp.asarray(mx), cp.asarray(my)) for mx, my in zip(maps_x, maps_y)]


_NVENC_AVAILABLE = None  # None = not yet tested, True/False after first call

def _check_nvenc_once():
    """Test NVENC exactly once at startup and cache the result."""
    ...
    try:
        ...
        import re
        m = re.search(r'minimum required Nvidia driver for nvenc is (\S+)', err)
        if m:
            print(f"[Encoder] libx264 (NVENC needs driver >= {m.group(1)}, update when convenient)")
        else:
            print("[Encoder] libx264 (NVENC unavailable)")
    except Exception:
        print("[Encoder] libx264 (NVENC check failed)")
    _NVENC_AVAILABLE = False
    return False


def _build_ffmpeg_cmd(output_path, W, H, fps):
    """Build FFmpeg command using NVENC if available, libx264 otherwise."""
    base = [
        'ffmpeg', '-y',
...
# ==========================================
# 4A. STATIC IMAGE STITCHER CLASS (Standalone)
# ==========================================
...
class SIFTInsta360ImageStitcher:
    def __init__(self, intrinsics_dir):
        ...
        # --- CONFIGURATION ---
        self.W = _cfg("output", "width", 1920)
        self.H = _cfg("output", "height", 960)
        ...
        self.Ks = []
        self.img_dims = []
        self.final_errors = [0.0] * 6 # Needed for pipeline metadata
        ...
        for i in range(1, 7):
            ...
            if not os.path.exists(path):
                raise FileNotFoundError(
                    f"[ERROR] Calibration file missing: {path}\n"
                    f"  Stitching with a generic camera matrix produces severe distortion artifacts.\n"
                    f"  Run the calibration step first to generate per-lens intrinsic files."
                )
    ...
    def check_alignment_only(self, frames):
        """Runs SIFT one last time just to PRINT errors (does not change geometry)."""
        print("\n--- VERIFYING ALIGNMENT (NO CORRECTION) ---")
        pixels_per_deg = self.W / 360.0
        
        warped = []
        for i in range(6):
            w = cv2.remap(frames[i], self.maps_x[i], self.maps_y[i], cv2.INTER_CUBIC)
            warped.append(w)
        ...
        for i in range(6):
            ...
            self.final_errors[i] = round(float(shift_deg), 3)
    ...
    def stitch_frames(self, frames, output_path, run_sift=True, save_media=True):
        ...
        if save_media:
            success = cv2.imwrite(output_path, final_pano, [cv2.IMWRITE_JPEG_QUALITY, 85])
            if success:
    ...
    def stitch_images(self, image_paths, output_path, run_sift=True):
        ...
        # --- OPTIMIZATION: BYPASS SIFT IF CACHED ---
        if run_sift:
            self.calibrate_angles_with_sift(frames)
        else:
            print("[INFO] SIFT Bypassed. Using cached daily geometry for instant stitching.")
...
# ==========================================
# 4B. VIDEO STITCHER CLASS (Standalone)
# ==========================================

class ClipVideoStitcher:
    def __init__(self, intrinsics_dir):
        self.intrinsics_dir = intrinsics_dir
    ...
    def calibrate_angles_with_sift(self, caps):
        ...
        # 1. Load frames once from VideoCaptures
        frames = []
        for cap in caps:
            ret, frame = cap.read()
            if not ret: return
    ...
    def check_drift_and_update(self, frames):
        ...
        pixels_per_deg = self.W / 360.0
        TARGET_MAX_ERROR = 0.70   # Don't stop until error is tiny
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            warped = [cv2.remap(frames[i], self.maps_x[i], self.maps_y[i], cv2.INTER_CUBIC) for i in range(6)]
    ...
    def stitch_video(self, input_paths, output_path):
        caps = []
        ...
        total_frames = int(caps[0].get(cv2.CAP_PROP_FRAME_COUNT)) or 0

        ffmpeg_cmd = _build_ffmpeg_cmd(output_path, self.W, self.H, fps)
        ...
        frame_idx = 0
        pbar = tqdm(total=total_frames, desc="Stitching", unit="frame",
                    dynamic_ncols=True) if total_frames > 0 else None
        try:
            while True:
                frames = []
                for cap in caps:
                ...
                # --- DRIFT CHECK EVERY 30 FRAMES (~1 second) ---
                if frame_idx > 0 and frame_idx % _cfg("processing", "drift_check_every_frames", 30) == 0:
                    if pbar: pbar.set_postfix_str("drift check...")
                    self.check_drift_and_update(frames)
                    if pbar: pbar.set_postfix_str("")

                # Remap all 6 lenses on GPU, then GPU-blend
                warpeds = [_gpu_remap(frames[i], self.maps_x[i], self.maps_y[i],
                           gpu_maps=self.gpu_maps[i] if self.gpu_maps else None)
                           for i in range(6)]
                final_pano = _blend_frames(warpeds, self.weights)

                # Burn frame number and timestamp for annotators
                timestamp_sec = frame_idx / fps
                label = f"Frame {frame_idx}  |  {timestamp_sec:.3f}s"
                cv2.putText(final_pano, label, (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 4, cv2.LINE_AA)
                cv2.putText(final_pano, label, (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 0), 2, cv2.LINE_AA)

                # Write frame with NVENC→libx264 fallback if encoder crashed
                if _nvenc_in_use and ffmpeg_proc.poll() is not None:
                    print("[WARNING] NVENC failed mid-encode.")
                    print("[CPU] Falling back to libx264...")
                    cpu_cmd = [
                        'ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo',
                        '-pix_fmt', 'bgr24', '-s', f'{self.W}x{self.H}', '-r', str(fps),
                ...
                try:
                    ...
                    pass  # frame dropped if pipe is broken
    ...
    def stitch_video_queued(self, frame_queues, output_path, fps, total_frames):
        """V4 pipeline mode: stitch from 6 live per-lens queues fed by undistortion threads.
        Each queue yields np.ndarray frames; a None sentinel signals end-of-stream for that lens.
        Stitching runs concurrently with undistortion — no waiting for all files to be written."""
        import queue as _q
        ffmpeg_cmd = _build_ffmpeg_cmd(output_path, self.W, self.H, fps)
        ffmpeg_proc = subprocess.Popen(ffmpeg_cmd, stdin=subprocess.PIPE,
                                       stderr=subprocess.DEVNULL)
        _nvenc_in_use = _check_nvenc_once()
        pbar = tqdm(total=total_frames, desc="Stitching (pipeline)",
                    unit="frame", dynamic_ncols=True) if total_frames > 0 else None
        frame_idx = 0

        while True:
            frames = []
            for q in frame_queues:
                frame = q.get()          # blocks until undistorter produces next frame
                if frame is None:        # sentinel — this lens stream is finished
                    frames = None
                    break
                frames.append(frame)
            if frames is None:
            ...
            if frame_idx > 0 and frame_idx % _cfg("processing", "drift_check_every_frames", 30) == 0:
                if pbar:
                    pbar.set_postfix_str("drift check...")
                self.check_drift_and_update(frames)
                if pbar:
            ...
            # Annotator timestamp overlay
            ts = frame_idx / fps
            label = f"Frame {frame_idx}  |  {ts:.3f}s"
            cv2.putText(final_pano, label, (10, 40), cv2.FONT_HERSHEY_SIMPLEX,
                        1.0, (0, 0, 0), 4, cv2.LINE_AA)
            cv2.putText(final_pano, label, (10, 40), cv2.FONT_HERSHEY_SIMPLEX,
                        1.0, (255, 255, 0), 2, cv2.LINE_AA)

            # Write to FFmpeg with NVENC→libx264 fallback
            if _nvenc_in_use and ffmpeg_proc.poll() is not None:
                print("[WARNING] NVENC failed mid-encode in pipeline mode, falling back to libx264...")
                cpu_cmd = [
                    'ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo',
                    '-pix_fmt', 'bgr24', '-s', f'{self.W}x{self.H}', '-r', str(fps),
                    '-i', 'pipe:', '-c:v', 'libx264',
                    '-preset', _cfg("output", "cpu_preset", "medium"),
                    '-crf', str(_cfg("output", "crf", 23)),
                    '-pix_fmt', 'yuv420p', output_path
                ]
                ffmpeg_proc = subprocess.Popen(cpu_cmd, stdin=subprocess.PIPE,
                                               stderr=subprocess.DEVNULL)
                _nvenc_in_use = False
            try:
                ffmpeg_proc.stdin.write(final_pano.tobytes())
    ...
    def process_video(self, video_path, output_path, json_path, meta, save_media=True):
        ...
        while cap.isOpened():
            ...
            results = self.model.track(
                ...
                iou=0.45, 
                imgsz=1920, 
                tracker=r"C:\IITM\Vehicle_detection_360\FINAL_custom_track.yaml"
            )[0]
            # -----------------------------
...
def run_stitching(output_dir, intr_dir, extracted_frames, meta_info, cached_stitcher=None, run_sift=True, save_media=True):
    ...
    stitcher = cached_stitcher if cached_stitcher is not None else SIFTInsta360ImageStitcher(intrinsics_dir=intr_dir)
...
def main():
    ...
    YOLO_PEDESTRIAN_MODEL = r"C:\IITM\yolo11n.pt"
